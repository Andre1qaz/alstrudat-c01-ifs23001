...
while read -r weight; do
  ...
  # Bandingkan output dengan expected output (dengan trim)
  if diff -q <(sed 's/[[:space:]]*$//' "$expected_file") <(sed 's/[[:space:]]*$//' "$output_file") > /dev/null; then
    echo "✅ Test case $index passed! (+$weight%)"
    total_score=$((total_score + weight))
  else
    echo "❌ Test case $index failed!"
    echo "=== Perbedaan (Expected vs Actual) ==="
    diff -u <(sed 's/[[:space:]]*$//' "$expected_file") <(sed 's/[[:space:]]*$//' "$output_file")
    echo "====================================="
  fi
